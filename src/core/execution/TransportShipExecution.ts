...
export class TransportShipExecution implements Execution {
  ...
  init(mg: Game, ticks: number) {
    ...
    this.pathFinder = PathFinder.Mini(
      mg, 
      mg.config().transportShipPathIterations(), 
  ...
  tick(ticks: number) {
    ...
    // Implement exponential backoff for failed pathfinding attempts
    if (this.pathFindingFailures > 0) {
      const backoffTicks = Math.min(30 * Math.pow(2, this.pathFindingFailures - 1), 300);
    ...
    switch (result.type) {
      ...
      case PathFindResultType.NextTile:
        ...
        this.pathFindingFailures = 0; // Reset failures on successful move
        break;
      ...
      case PathFindResultType.PathNotFound:
        ...
        if (this.pathFindingFailures >= 3) {
          console.warn(`path not found to dst after ${this.pathFindingFailures} attempts`);
          this.attacker.addTroops(this.troops);
          this.boat.delete(false);
          this.active = false;
        }
