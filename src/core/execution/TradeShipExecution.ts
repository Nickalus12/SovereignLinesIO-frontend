...
export class TradeShipExecution implements Execution {
  ...
  private cachedPath: TileRef[] | null = null;
  ...
  init(mg: Game, ticks: number): void {
    ...
    this.pathFinder = PathFinder.Mini(
      mg,
      mg.config().tradeShipPathIterations(),
  ...
  tick(ticks: number): void {
    ...
    if (this.pathFindingFailures > 0) {
      const backoffTicks = Math.min(20 * Math.pow(2, this.pathFindingFailures - 1), 200);
      if (ticks - this.lastPathFindingAttempt < backoffTicks) {
        return;
      }
    }

    this.lastPathFindingAttempt = ticks;
    ...
    switch (result.type) {
      ...
      case PathFindResultType.PathNotFound:
        ...
        if (this.pathFindingFailures >= 3) {
          console.warn(`trade ship cannot find route after ${this.pathFindingFailures} attempts`);
          if (this.tradeShip.isActive()) {
            this.tradeShip.delete(false);
          }
          this.active = false;
