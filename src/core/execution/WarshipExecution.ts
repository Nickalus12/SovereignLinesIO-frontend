...
export class WarshipExecution implements Execution {
  ...
  private pathFindingFailures = 0;
  private lastPathFindingAttempt = 0;
  private cachedRandomTile: TileRef | undefined;
  private cacheExpiryTicks = 0;
  ...
  init(mg: Game, ticks: number): void {
    this.mg = mg;
    // Use configurable iterations for better performance
    this.pathfinder = PathFinder.Mini(
      mg,
      mg.config().warshipPathIterations(),
      mg.config().pathfindingMaxRetries()
    );
  ...
  private huntDownTradeShip() {
    // Implement exponential backoff for failed pathfinding
    if (this.pathFindingFailures > 0) {
      const backoffTicks = Math.min(10 * Math.pow(2, this.pathFindingFailures - 1), 100);
      if (this.mg.ticks() - this.lastPathFindingAttempt < backoffTicks) {
        return;
      }
    }

    this.lastPathFindingAttempt = this.mg.ticks();
    for (let i = 0; i < 2; i++) {
      ...
      switch (result.type) {
        ...
        case PathFindResultType.NextTile:
          ...
          this.pathFindingFailures = 0; // Reset on successful move
          break;
        ...
        case PathFindResultType.PathNotFound:
          ...
          if (this.pathFindingFailures >= 3) {
            console.log(`path not found to target after ${this.pathFindingFailures} attempts`);
            this.warship.setTargetUnit(undefined);
  ...
  private patrol() {
    ...
    // Skip pathfinding if we're already at or near the target
    const distToTarget = this.mg.euclideanDistSquared(
      this.warship.tile(),
      this.warship.targetTile()!
    );
    if (distToTarget <= 4) { // Within 2 tiles
    ...
    switch (result.type) {
      ...
      case PathFindResultType.PathNotFound:
        ...
        if (this.pathFindingFailures >= 2) {
          console.warn(`path not found to target tile after ${this.pathFindingFailures} attempts`);
          this.warship.setTargetTile(undefined);
          this.pathFindingFailures = 0;
        }
        break;
  ...
  randomTile(allowShoreline: boolean = false): TileRef | undefined {
    // Cache random tile for 50 ticks to reduce computation
    if (this.cachedRandomTile && this.mg.ticks() < this.cacheExpiryTicks) {
      return this.cachedRandomTile;
    }

    let warshipPatrolRange = this.mg.config().warshipPatrolRange();
    const maxAttemptBeforeExpand: number = 100; // Reduced from 500
    let attempts: number = 0;
    let expandCount: number = 0;
    while (expandCount < 2) { // Reduced from 3
      const x =
      ...
      this.cachedRandomTile = tile;
      this.cacheExpiryTicks = this.mg.ticks() + 50;
      return tile;
    }
    console.warn(
