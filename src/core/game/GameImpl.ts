...
export class GameImpl implements Game {
  ...
  constructor(
    ...
    // Party alliances in FFA mode will be created after spawn phase ends
  }
  ...
  private createPartyAlliances() {
    // Only create alliances in FFA mode
    if (this._config.gameConfig().gameMode !== GameMode.FFA) {
      return;
    }
    
    console.log("Creating party alliances for FFA game after spawn phase");
    
    // Group players by party ID
    const partyGroups = new Map<string, PlayerImpl[]>();
    
    for (const player of this._playersBySmallID) {
      if (player instanceof PlayerImpl) {
        const playerInfo = player.info();
        console.log(`Player ${playerInfo.name} has partyId: ${playerInfo.partyId}`);
        
        const partyId = playerInfo.partyId;
        if (partyId) {
          if (!partyGroups.has(partyId)) {
            partyGroups.set(partyId, []);
          }
          const group = partyGroups.get(partyId);
          if (group) {
            group.push(player);
          }
        }
    ...
    console.log(`Found ${partyGroups.size} party groups`);
    
    // Create alliances between all party members
    for (const [partyId, partyMembers] of partyGroups) {
      console.log(`Party ${partyId} has ${partyMembers.length} members`);
      
      if (partyMembers.length < 2) {
        console.log(`Skipping party ${partyId} - not enough members`);
        continue;
      }
      
      // Create alliances between each pair of party members
      for (let i = 0; i < partyMembers.length; i++) {
        for (let j = i + 1; j < partyMembers.length; j++) {
          const player1 = partyMembers[i];
          const player2 = partyMembers[j];
          
          // Create alliance without going through the request system
          ...
          // Update relations to maximum friendly
          player1.updateRelation(player2, 100);
          player2.updateRelation(player1, 100);
          
          // Log the alliance creation
          console.log(`Created party alliance between ${player1.name()} and ${player2.name()} in party ${partyId}`);
        }
      }
    }
  }
  ...
  executeNextTick(): GameUpdates {
    ...
    // Check if we're transitioning out of spawn phase
    const wasInSpawnPhase = this.inSpawnPhase();
    this._ticks++;
    const nowInSpawnPhase = this.inSpawnPhase();
    
    // If we just exited spawn phase, create party alliances
    if (wasInSpawnPhase && !nowInSpawnPhase) {
      console.log("Spawn phase ended, creating party alliances");
      this.createPartyAlliances();
    }
