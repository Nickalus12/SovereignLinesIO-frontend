...
export class UnitImpl implements Unit {
  ...
  private _deployedAt: number | undefined;
  private _suppliesRemaining: number | undefined;
  constructor(
    ...
    this._readyMissileCount =
      "readyMissileCount" in params ? (params.readyMissileCount ?? 1) : 1;
    this._deployedAt =
      "deployedAt" in params ? (params.deployedAt ?? undefined) : undefined;
    this._suppliesRemaining =
      "suppliesRemaining" in params ? (params.suppliesRemaining ?? undefined) : undefined;
  ...
  toUpdate(): UnitUpdate {
    return {
      ...
      suppliesRemaining: this._suppliesRemaining,
    };
  }
  ...
  increaseLevel(): void {
    ...
    if (this.type() === UnitType.SAMLauncher) {
      // For SAMs, set missile count based on level configuration
      this._readyMissileCount = this.mg.config().samMissilesPerLevel(this._level);
    } else if (this.type() === UnitType.MissileSilo) {
      // Missile silos still get +1 per level
      this._readyMissileCount++;
    }
    this.mg.addUpdate(this.toUpdate());
  }
  ...
  setDeployedAt(value: number): void {
    this._deployedAt = value;
    this.mg.addUpdate(this.toUpdate());
  }

  suppliesRemaining(): number | undefined {
    return this._suppliesRemaining;
  }

  setSuppliesRemaining(value: number): void {
    this._suppliesRemaining = value;
    this.mg.addUpdate(this.toUpdate());
  }
}
