...
export class PlayerImpl implements Player {
  ...
  public buildableUnits(tile: TileRef): BuildableUnit[] {
    return Object.values(UnitType).map((u) => {
      // For SAMs, calculate valid tiles specifically for SAM placement
      const validTiles = u === UnitType.SAMLauncher 
        ? this.validStructureSpawnTiles(tile, u)
  ...
  canBuild(
    ...
    switch (unitType) {
      ...
      case UnitType.SAMLauncher:
        // If validTiles not provided, calculate them specifically for SAMs
        const samValidTiles = validTiles ?? this.validStructureSpawnTiles(targetTile, unitType);
        return this.landBasedStructureSpawn(targetTile, samValidTiles, unitType);
      case UnitType.SupplyTruck:
        return this.supplyTruckSpawn(targetTile);
      default:
  ...
  landBasedStructureSpawn(
    ...
    const tiles = validTiles ?? this.validStructureSpawnTiles(tile, unitType);
    if (tiles.length === 0) {
      return false;
    }
    // For SAMs, we need to check if the exact requested tile is valid
    // Other structures can be placed at the nearest valid tile
    if (unitType === UnitType.SAMLauncher) {
      // Check if the requested tile is in the valid tiles list
      for (const validTile of tiles) {
        if (validTile === tile) {
          return tile;
        }
      }
      return false; // Requested tile is not valid for SAM placement
    }
    return tiles[0];
  }

  private validStructureSpawnTiles(tile: TileRef, unitType?: UnitType): TileRef[] {
    if (this.mg.owner(tile) !== this) {
      return [];
    }
    ...
    // Special handling for SAM launchers - check distance to other SAMs
    if (unitType === UnitType.SAMLauncher) {
      const samMinDistSquared = this.mg.config().samMinDist() ** 2;
      // Check both existing SAMs and constructions that will become SAMs
      const nearbySAMs = this.mg
        .nearbyUnits(tile, this.mg.config().samMinDist() * 2, [UnitType.SAMLauncher, UnitType.Construction])
        .map((u) => u.unit)
        .filter(u => u.type() === UnitType.SAMLauncher || 
                    (u.type() === UnitType.Construction && u.constructionType() === UnitType.SAMLauncher));
      
      for (const t of nearbyTiles) {
        // Check distance to other SAMs specifically
        for (const sam of nearbySAMs) {
          if (this.mg.euclideanDistSquared(sam.tile(), t) < samMinDistSquared) {
            validSet.delete(t);
            break;
          }
        }
        // Also check regular structure distance
        if (validSet.has(t)) {
      ...
      // Regular structure distance checking
      for (const t of nearbyTiles) {
        for (const unit of nearbyUnits) {
          if (this.mg.euclideanDistSquared(unit.tile(), t) < minDistSquared) {
            validSet.delete(t);
  ...
  supplyTruckSpawn(targetTile: TileRef): TileRef | false {
    // Supply trucks can only be built on land tiles owned by the player
    if (!this.mg.isLand(targetTile) || this.mg.owner(targetTile) !== this) {
      return false;
    }

    // Check the no-stacking rule: Supply trucks cannot be within 50 tiles of each other
    const NO_STACK_RADIUS = 50;
    const nearbyTrucks = this.units(UnitType.SupplyTruck)
      .filter(truck => this.mg.manhattanDist(targetTile, truck.tile()) < NO_STACK_RADIUS);
    
    if (nearbyTrucks.length > 0) {
      return false;
    }

    // Also check for Supply Trucks under construction
    const nearbyConstructions = this.units(UnitType.Construction)
      .filter(construction => 
        construction.constructionType() === UnitType.SupplyTruck &&
        this.mg.manhattanDist(targetTile, construction.tile()) < NO_STACK_RADIUS
      );
    
    if (nearbyConstructions.length > 0) {
      return false;
    }
