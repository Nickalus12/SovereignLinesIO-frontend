...
export class DefaultConfig implements Config {
  ...
  supplyTruckRange(): number {
    return 30;
  }
  supplyTruckAttackBonus(): number {
    return 1.1; // 10% attack bonus
  }
  supplyTruckDefenseBonus(): number {
    return 1.15; // 15% defense bonus
  }
  supplyTruckSupplyDuration(): number {
    return 180 * 10; // 3 minutes of supplies
  }
  supplyTruckMoveSpeed(): number {
    return 2; // Not used - supply trucks are stationary
  }
  ...
  attackLogic(
    ...
    // Check for supply truck bonus for attacker
    if (attacker.isPlayer()) {
      for (const st of gm.nearbyUnits(
        tileToConquer,
        gm.config().supplyTruckRange(),
        UnitType.SupplyTruck,
      )) {
        if (st.unit.owner() === attacker || st.unit.owner().isFriendly(attacker)) {
          // Supply trucks provide attack bonus
          mag /= this.supplyTruckAttackBonus();
          speed /= this.supplyTruckAttackBonus();
          break;
        }
      }
    }

    // Check for supply truck defense bonus for defender
    if (defender.isPlayer()) {
      for (const st of gm.nearbyUnits(
        tileToConquer,
        gm.config().supplyTruckRange(),
        ...
        if (st.unit.owner() === defender || (defender as Player).isFriendly(st.unit.owner())) {
          // Supply trucks provide defense bonus
          mag *= this.supplyTruckDefenseBonus();
          speed *= this.supplyTruckDefenseBonus();
          break;
        }
      }
    }
  ...
  samMinDist(): number {
    // SAMs must be at least 35 tiles apart to prevent clustering
    return 35;
  }
  ...
  samMissilesPerLevel(level: number): number {
    // Level 1-2: 1 missile
    // Level 3+: 2 missiles
    // Easy to adjust these thresholds
    if (level >= 3) {
      return 2;
    }
    return 1;
  ...
  transportShipPathIterations(): number {
    return 1000; // Reduced from 10,000
  }

  warshipPathIterations(): number {
    return 1000; // Reduced from 5,000
  }

  tradeShipPathIterations(): number {
    return 500; // Reduced from 2,500
  }

  pathfindingMaxRetries(): number {
