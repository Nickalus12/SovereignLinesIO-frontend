...
const frequency: Partial<Record<keyof typeof GameMapType, number>> = {
  World: 3,
  Europe: 2,
  Africa: 2,
  Australia: 1,
...
export class MapPlaylist {
  ...
  public gameConfig(preferredMode?: GameMode): GameConfig {
    const { map, mode } = preferredMode ? this.getNextMapWithMode(preferredMode) : this.getNextMap();
  ...
  private getNextMapWithMode(preferredMode: GameMode): MapWithMode {
    // Get a random map
    const maps = Object.values(GameMapType);
    const randomMap = maps[Math.floor(Math.random() * maps.length)];
    return { map: randomMap, mode: preferredMode };
  }

  private shuffleMapsPlaylist(): boolean {
    const maps: GameMapType[] = [];
    Object.keys(GameMapType).forEach((key) => {
      const freq = frequency[key] || 0;
