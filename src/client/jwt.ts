...
export async function discordLogin() {
  ...
  try {
    ...
    if (!response || !response.ok) {
      alert('Discord login is not available in development mode.\n\nTo enable Discord login:\n1. Set up the API server\n2. Configure Discord OAuth credentials\n3. Run the API server on port 8787');
      console.error('API server not available at', apiBase);
      return;
    }
  } catch (error) {
    alert('Discord login server is not available. Please try again later.');
