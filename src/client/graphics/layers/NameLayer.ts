...
export class NameLayer implements Layer {
  ...
  public init() {
    ...
    this.container.style.zIndex = "11"; // Just above game canvas (10) but below all UI
    this.container.id = "name-layer-container";
  ...
  renderPlayerInfo(render: RenderInfo) {
    ...
    // Lower threshold for name visibility to show names at more zoom levels
    if (size < 4 || !this.transformHandler.isOnScreen(render.location)) {
  ...
  public cleanup() {
    ...
    this.renders.forEach(render => {
      if (render.element && render.element.parentNode) {
        render.element.remove();
      }
    });
    this.renders = [];
