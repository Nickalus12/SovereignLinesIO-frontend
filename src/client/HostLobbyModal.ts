...
export class HostLobbyModal extends LitElement {
                        ...
                        return html`
                          ...
                          >
                            <map-display
                              ...
                              .translation=${mapKey ? translateText(
  ...
  public open() {
    // Dispatch event that host is creating a lobby
    window.dispatchEvent(new CustomEvent("host-creating-lobby", {
      bubbles: true,
      composed: true
    }));
    ...
    createLobby()
      ...
      .then(() => {
        // Send initial game config after lobby is created
        this.putGameConfig();
      });
    this.modalEl?.open();
    this.playersInterval = setInterval(() => this.pollPlayers(), 1000);
    // Force update to ensure translations are loaded
    this.requestUpdate();
    // Additional update after a small delay to ensure translations are fully loaded
    setTimeout(() => this.requestUpdate(), 50);
  }

  public show() {
    // Alias for open() to match the PartySystem's call
    this.open();
  }
  ...
  public close() {
    ...
    // Don't reset state - keep user's selections
  }

  private resetState() {
    // Reset to default values for next time modal opens
  ...
  private async handleGameModeSelection(value: GameMode) {
    console.log(`Selecting game mode: ${GameMode[value]} (${value})`);
    this.gameMode = value;
  ...
  private async putGameConfig() {
    ...
    const response = await fetch(
      `${window.location.origin}/${config.workerPath(this.lobbyId)}/api/game/${this.lobbyId}`,
      {
        ...
        body: JSON.stringify(gameConfig satisfies Partial<GameConfig>),
      },
    );
    return response;
  }
