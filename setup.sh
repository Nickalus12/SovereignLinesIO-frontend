...
echo "👤 Setting up sovereign-lines user..."
# Create sovereign-lines user if it doesn't exist
if id "sovereign-lines" &> /dev/null; then
    echo "User sovereign-lines already exists"
else
    useradd -m -s /bin/bash sovereign-lines
    echo "User sovereign-lines created"
fi

# Check if sovereign-lines is already in docker group
if groups sovereign-lines | grep -q '\bdocker\b'; then
    echo "User sovereign-lines is already in the docker group"
else
    # Add sovereign-lines to docker group
    usermod -aG docker sovereign-lines
    echo "Added sovereign-lines to docker group"
fi

# Create .ssh directory for sovereign-lines if it doesn't exist
if [ ! -d "/home/sovereign-lines/.ssh" ]; then
    mkdir -p /home/sovereign-lines/.ssh
    chmod 700 /home/sovereign-lines/.ssh
    echo "Created .ssh directory for sovereign-lines"
fi

# Copy SSH keys from root if they exist and haven't been copied yet
if [ -f /root/.ssh/authorized_keys ] && [ ! -f /home/sovereign-lines/.ssh/authorized_keys ]; then
    cp /root/.ssh/authorized_keys /home/sovereign-lines/.ssh/
    chmod 600 /home/sovereign-lines/.ssh/authorized_keys
    echo "SSH keys copied from root to sovereign-lines"
fi
...
# Set proper ownership for sovereign-lines's home directory
chown -R sovereign-lines:sovereign-lines /home/sovereign-lines
echo "Set proper ownership for sovereign-lines's home directory"

# Create directory for OpenTelemetry configuration
echo "📊 Setting up Node Exporter and OpenTelemetry Collector..."
OTEL_CONFIG_DIR="/home/sovereign-lines/otel"
        ...
        - job_name: 'node'
          ...
          relabel_configs:
            - source_labels: [__address__]
              regex: '.*'
              target_label: sovereign-lines.host
...
chown -R sovereign-lines:sovereign-lines "$OTEL_CONFIG_DIR"
...
echo "🎉 SETUP COMPLETE!"
echo "====================================================="
echo "The sovereign-lines user has been set up and has Docker permissions."
